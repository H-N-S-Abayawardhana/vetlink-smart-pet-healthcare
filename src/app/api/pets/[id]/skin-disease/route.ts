...
async function assertPetAccess(petId: string, session: any) {
  ...
  const allowed = ownerIdStr === userIdStr || userRole === 'SUPER_ADMIN' || userRole === 'VETERINARIAN';
  if (!allowed) {
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await pool.query('SELECT skin_disease_history FROM pets WHERE id = $1', [id]);
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const allProbabilities =
      typeof allProbRaw === 'string' ? safeJsonParse(allProbRaw) : safeJsonParse(String(allProbRaw || ''));

    // Save image to public/uploads/skin-disease
    const uploadsDir = path.join(process.cwd(), 'public', 'uploads', 'skin-disease');
    ...
    const ownerId = access.petRow.owner_id ? String(access.petRow.owner_id) : null;
    if (!ownerId) return NextResponse.json({ error: 'Pet owner not found' }, { status: 400 });
