...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, address, location, contact, delivery = {}, inventory = [] } = body;

    if (!name || !location || !location.lat || !location.lng) {
      return NextResponse.json({ error: 'Missing required fields: name, location' }, { status: 400 });
    }

    const data = await readData();
    const pharmacies = data.pharmacies || [];
    const nextId = pharmacies.length > 0 ? Math.max(...pharmacies.map((p: any) => p.id)) + 1 : 1;
    ...
    return NextResponse.json({ success: true, pharmacy: newPharmacy }, { status: 201 });
  } catch (error) {
