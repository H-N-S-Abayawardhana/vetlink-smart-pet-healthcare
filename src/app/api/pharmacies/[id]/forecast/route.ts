...
const SALES_DATA = path.join(process.cwd(), 'src', 'data', 'pharmacy-sales.json');
...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!pharm || !sales) return NextResponse.json({ error: 'Data missing' }, { status: 500 });
    ...
    (pharmacy.inventory || []).forEach((i: any) => {
      ...
      if ((i.stock || 0) < Math.ceil(forecastNext30 * 0.5)) {
        alerts.push({ type: 'low_stock', medication: i.name, stock: i.stock, recommended: optimal });
      }
      ...
      if (i.expiry) {
        ...
        if (daysLeft <= 60) {
          alerts.push({ type: 'expiring', medication: i.name, daysLeft, expiry: i.expiry });
        }
      }

      recommendations.push({ medication: i.name, stock: i.stock, forecastNext30, optimalStock: optimal });
    });

    // Sales overview
    const totalRevenue = (sales.sales || []).reduce((acc: number, s: any) => acc + ((s.price || 0) * (s.quantity || 0)), 0);

    return NextResponse.json({ success: true, pharmacyId: pid, recommendations, alerts, salesOverview: { totalRevenue } });
  } catch (err) {
    console.error(err);
