...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ success: true, inventory: pharmacy.inventory || [] });
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!name || !form) return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
    ...
    const nextId = inv.length > 0 ? Math.max(...inv.map((i: any) => i.id)) + 1 : 1;
    const newItem = { id: nextId, name, form, strength: strength || '', stock, expiry, price };
