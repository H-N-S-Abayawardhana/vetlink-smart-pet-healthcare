  ...
  try {
    ...
    pharmacies.forEach((p: any) => {
      ...
      for (const it of items) {
        ...
        const found = (p.inventory || []).find((inv: any) => (inv.name || '').toLowerCase() === name);
        if (!found || (found.stock || 0) < qty) {
          hasAll = false;
          break;
        }

        matchedItems.push({ medication: found.name, qty, unitPrice: found.price, total: (found.price || 0) * qty });
      ...
      if (hasAll) {
        ...
        if (lat != null && lng != null && p.location) {
          distance = haversineDistance(Number(lat), Number(lng), Number(p.location.lat), Number(p.location.lng));
        }
    ...
    return NextResponse.json({ error: 'Failed to match pharmacies' }, { status: 500 });
  }
}
