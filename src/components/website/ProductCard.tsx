  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex flex-col">
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="text-lg font-semibold text-gray-900">{formatLKR(product.price)}</div>
        <div>
          {product.stock > 0 ? (
            <button onClick={() => onAddToCart && onAddToCart(product)} className="px-3 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700">Add</button>
          ) : (
            <div className="px-3 py-1 text-xs text-red-600 bg-red-50 rounded">Out of stock</div>
          )}
        </div>
      </div>
    </div>
