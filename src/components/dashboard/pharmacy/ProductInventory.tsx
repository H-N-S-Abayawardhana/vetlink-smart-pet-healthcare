...
export default function ProductInventory() {
  ...
  const getStockStatus = (stockLevel: number, reorderPoint: number) => {
    if (stockLevel === 0) return { text: 'Out of Stock', color: 'bg-red-100 text-red-800' };
    if (stockLevel <= reorderPoint) return { text: 'Low Stock', color: 'bg-yellow-100 text-yellow-800' };
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600">Manage and track pharmaceutical products</p>
      </div>
              ...
              return (
                <tr key={product.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-center">
                    <div className="font-semibold text-gray-900">{product.stockLevel}</div>
                  ...
                  <td className="py-3 px-4">
                    <div className={`text-sm ${expiringSoon ? 'text-orange-600 font-medium' : 'text-gray-700'}`}>
                      {product.expiryDate}
                    </div>
                    {expiringSoon && (
                      <div className="text-xs text-orange-600">⚠️ Expiring soon</div>
                    )}
                  </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
        <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
          <div className="text-sm text-blue-600 font-medium mb-1">Total Products</div>
        ...
        <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
          <div className="text-sm text-yellow-600 font-medium mb-1">Low Stock</div>
          <div className="text-2xl font-bold text-yellow-900">
            {products.filter(p => p.stockLevel > 0 && p.stockLevel <= p.reorderPoint).length}
          </div>
        </div>
        <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
          <div className="text-sm text-orange-600 font-medium mb-1">Expiring Soon</div>
